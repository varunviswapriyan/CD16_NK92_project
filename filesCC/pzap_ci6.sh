...
if len(nulls) > 1:
    print()
    nrep = '?'
    for d in sorted(glob.glob(os.path.join(ROOT, 'nul*'))):
        f = os.path.join(d, SUB, 'v_config.json')
        if os.path.exists(f):
            nrep = json.load(open(f)).get('N_REPS', '?'); break
    print('QUALITY CHECK (not applied to the CI above).  %d refits of the ORIGINAL'
          % len(nulls))
    print('data with fresh optimizer randomness at N_REPS=%s.  Note the null SD is' % nrep)
    print('unweighted while the bootstrap SD is probability-weighted, so read the')
    print('percentages as indicative rather than exact:')
    for n in FIT6:
...
# would be resubmitted; build_one writes a fresh one for each job it makes
rm -f $ROOT/run_*.sh 2>/dev/null || true
...
todo=($ROOT/run_*.sh)
if [ ${#todo[@]} -eq 0 ]; then
  echo "nothing to submit -- every sample already holds a finished fit."
else
