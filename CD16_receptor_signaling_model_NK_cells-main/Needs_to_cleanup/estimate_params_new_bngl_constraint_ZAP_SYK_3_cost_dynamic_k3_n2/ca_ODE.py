...
def calcium(y, t, PZAP,C1,C2,g,k3):
    ...
    dhdt=(C2*((PZAP**nn/(PZAP**nn + k3**nn))))*((s/(s+ca ** 2))-h)

    dydt = [dcadt,dhdt]
    return dydt

def solve(tnew,N,y0,PZAP,C1,C2,g,k3):   
# store solution
    ...
    for i in range(1,N):
        ...
        # solve for next step
        sol = odeint(calcium,y0,tspan,args=(PZAP[i],C1,C2,g,k3))
