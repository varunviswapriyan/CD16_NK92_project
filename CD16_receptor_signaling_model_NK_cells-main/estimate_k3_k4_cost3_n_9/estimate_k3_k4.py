...
def ca_cost_func(param):
    ...
    # Convert from log10 space for all 5 parameters
    param_converted = 10 ** param  # Convert C1, C2, g, k3, k4 from log space
    ...
    for i in range(n_particles):
        try:
            ...
            k3 = param_converted[i, 3]
            k4 = param_converted[i, 4]
            ...
            if i % 5 == 0:  # Print every 5th particle to reduce output
                print(f"Particle {i}: C1={C1:.2e}, C2={C2:.2e}, g={g:.2e}, k3={k3:.2e}, k4={k4:.2e}")
...
def main():
    ...
    # Parameter bounds for optimization (in log10 space for all 5 parameters)
    # [log10(C1), log10(C2), log10(g), log10(k3), log10(k4)]
    
    # Bounds for optimization parameters (log10 space)
    lb = [2.0,   # log10(C1) - lower bound around 1e3
          -2.0,  # log10(C2) - lower bound around 1e-3  
          -4.0,  # log10(g) - lower bound around 1e-4
          -1.3,  # log10(k3) - lower bound around 1e-3
    ...
    ub = [6.0,   # log10(C1) - upper bound around 1e6
          ...
          -0.8,   # log10(k3) - upper bound around 100
          +1.0]   # log10(k4) - upper bound around 100

    print(f"Parameter bounds (all in log10 space):")
    print(f"Lower bounds: {lb}")
    print(f"Upper bounds: {ub}")

    # Initialize PSO optimizer
    optimizer = GlobalBestPSO(n_particles=number_of_particles, 
                              dimensions=5,  # Only 5 parameters to optimize
    ...
    k3_opt = 10**optimized_param[3]
    k4_opt = 10**optimized_param[4]
    ...
    try:
        ...
        with open(f"analysis_param_residue.dat", 'w') as f:
            f.write(f"Optimization Results - 5 Parameters (C1, C2, g, k3, k4)\n")
            ...
            f.write(f"k3_optimal = {k3_opt:.6e}\n")
        ...
        with open("common_parameters.dat", 'a') as f3:
            ...
            f3.write(f"{optimized_param[3]:.6f}\t{optimized_param[4]:.6f}\t{residue_small:.6f}\n")
