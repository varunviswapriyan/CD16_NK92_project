...
--recdeg   : replace the 4 per-bound-kinase CBL rules with per-ligated-receptor rule(s)   [rule change]
--itamnorm : keep the CBL rules, divide kdl by the receptor's ITAM count (6/2/4)          [parameter only]
--delmol   : keep the CBL rules, add DeleteMolecules so only the tagged kinase is removed [smaller rule change]
...
if '--itamnorm' in flags:
    n_itam = {'zeta': 6.0, 'gamma': 2.0, 'mixed': 4.0}[kind]
    hits = [i for i, l in enumerate(L) if re.match(r'^\s*kdl\s', l)]
    if len(hits) != 1: die('kdl line not found for itamnorm')
    L[hits[0]] = L[hits[0]].rstrip() + ' / ITAM_N'
    L.insert(hits[0], f'ITAM_N {n_itam}')
    log.append(f'kdl /= ITAM_N ({n_itam:g})')

if '--delmol' in flags:
    n = 0
    for i, l in enumerate(L):
        if '_bound_CBL_degrade_' in l:
            new, k = re.subn(r'->\s*ligand\(receptor\)\s+kdl\s*$', '-> 0 kdl DeleteMolecules', l.rstrip())
            if k != 1: die(f'unexpected CBL rule format: {l}')
            L[i] = new; n += 1
    if n != 4: die(f'expected 4 CBL degrade rules, found {n}')
    log.append('CBL rules: DeleteMolecules (kinase only removed, ligand stays bound)')
